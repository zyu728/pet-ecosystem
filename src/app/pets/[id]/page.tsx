  ...
  return (
    <div className="pb-20">
      ...
      <div className="h-56 bg-gradient-to-b from-orange-100 to-orange-50 flex flex-col items-center justify-center relative">
        ...
        {isOwner && (
          <button onClick={() => router.push(`/pets/${pet.id}/edit`)} className="absolute top-4 right-4 bg-white/80 px-3 py-1 rounded-full text-sm shadow">编辑</button>
        )}
        ...
        <div className="bg-white rounded-2xl shadow-lg p-5 space-y-3">
          ...
          <div className="bg-blue-50 rounded-xl p-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-blue-600">{collar ? `在线 · 电量 ${collar.battery_level}%` : '未绑定项圈'}</p>
            </div>
            {collar && (
              <button onClick={() => router.push(`/pets/tracking?petId=${pet.id}`)} className="bg-blue-500 text-white px-4 py-1.5 rounded-full text-sm font-medium">实时追踪</button>
            )}
          </div>
          ...
          <div className="bg-green-50 rounded-xl p-4 flex items-center justify-between">
            <div>
              <p className="font-medium text-green-900">🩺 健康档案</p>
              <p className="text-xs text-green-600">疫苗日历 · 就诊记录 · 体重曲线</p>
            </div>
            <button onClick={() => router.push(`/pets/${pet.id}/health`)} className="bg-green-500 text-white px-4 py-1.5 rounded-full text-sm font-medium">查看档案</button>
          </div>
          ...
          {pet.vaccine_records && pet.vaccine_records.length > 0 && (
            <div>
              ...
              {(pet.vaccine_records as any[]).map((v: any, i: number) => {
                ...
                return (
                  <div key={i} className={`text-sm flex justify-between py-1.5 px-2 rounded-lg ${isUrgent ? 'bg-red-50 text-red-600 font-medium' : 'text-gray-600'}`}>
