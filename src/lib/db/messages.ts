...
export async function getConversations(userId: string): Promise<any[]> {
  ...
  // Single query with explicit join columns to get both participants' info
  const { data, error } = await supabase
    .from('conversations')
    .select(`
      *,
      p1:profiles!conversations_participant_1_fkey(id,nickname,avatar_url),
      p2:profiles!conversations_participant_2_fkey(id,nickname,avatar_url)
    `)
  ...
  return data.map((conv: any) => ({
    ...conv,
    other_user: conv.participant_1 === userId ? conv.p2 : conv.p1,
  }))
}
...
export async function getOrCreateConversation(user1Id: string, user2Id: string): Promise<string | null> {
  ...
  const { data: existing } = await supabase
    .from('conversations').select('id').eq('participant_1', p1).eq('participant_2', p2).single()
...
export async function sendMessage(conversationId: string, senderId: string, content: string, imageUrl?: string): Promise<Message | null> {
  const supabase = createClient()
  const { data, error } = await supabase
    .from('messages').insert({ conversation_id: conversationId, sender_id: senderId, content, image_url: imageUrl || null }).select().single()
